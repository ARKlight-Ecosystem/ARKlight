"""
`log2` derivation fragment (`v0.064`, JS vocabulary addendum stage
4/10 -- see `docs/version history/v0.064.md`). See `sum.py` for the
general shape.

Single-name math transform: `Math.log2`.

Mirrors `arklight.ir.build`'s build-time `_evaluate_derivation
("log2", ...)` case (via `arklight/ir/js_numeric.py` where Python's
...
JS_FRAGMENT = """    log2: function (state, names, args) {
      return Math.log2(Number(state[names[0]]) || 0);
    }"""
