"""
`sign` derivation fragment (`v0.064`, JS vocabulary addendum stage
4/10 -- see `docs/version history/v0.064.md`). See `sum.py` for the
general shape.

Single-name math transform: `Math.sign` (`-1`, `0`, or `1`).

Mirrors `arklight.ir.build`'s build-time `_evaluate_derivation
("sign", ...)` case (via `arklight/ir/js_numeric.py` where Python's
...
JS_FRAGMENT = """    sign: function (state, names, args) {
      return Math.sign(Number(state[names[0]]) || 0);
    }"""
