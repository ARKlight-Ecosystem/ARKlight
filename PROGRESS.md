...
| v0.060-stage0 | User-defined components, Stage 0 of 4: registration API (`component`/`Prop`), props contract, Option A macro expansion pass, cycle/depth guards, experimental `mode="registry"` selector -- `USER-DEFINED-COMPONENTS-IMPLEMENTATION.md` (retired -- see `CHANGELOG.md`; design record: `docs/Foundational/USER-DEFINED-COMPONENTS.md`) | DONE |
...
| v0.064   | `arklight search --retrieve-doc` -- doc-tree retrieval mode (`--foundational`/`--backends`/`--proposals`/`--implementation`/`--js-backend` (removed later, along with its folder)/`--far-future`/`--version-history`, plus `--file NAME`) on the existing `search` subcommand, fully wired into `arklight/cli/main.py`/`arklight/cli/doc_retrieval.py`; `tests/test_doc_retrieval.py` (48 tests incl. `test_cli.py`) passing. A single, self-contained CLI addition, with no separate implementation addendum -- see `docs/Foundational/DESIGN-NOTES.md` for the accepted proposal this shipped from | DONE |
...
| v0.06503 | Capability fix: live-input -> action-value -- `Bind("name")` accepted as `Action.set`/`Action.append`'s `value` (`{"__state__": name}` marker, opt-in per argument via `ActionSpec.state_args`, build-time validated, resolved by `resolveActionArgs` at dispatch time), so `[type a task] [Add]` is expressible with `bind_value=Bind.model(...)` + `Watch(..., then=Action.reset(...))`. Issue-register #7; `docs/Foundational/DESIGN-NOTES.md`. Also fixes a raw `TypeError` on `Action.append(name, Bind(...))`. `tests/test_action_value_from_state.py` (31 tests); full suite 1538 passed. `0.06502` -> `0.06503`; roadmap `v0.065` untouched. Out-of-band, same slot-sharing precedent | DONE |
| v0.06504 **(no slot -- confirmed by maintainer)** | Bug fix: `trusted_script_origins` CSP directive injection -- `_render_csp_meta_tag` spliced entries verbatim into `script-src`; `'unsafe-inline'`/`'unsafe-eval'` (quoted or not) and directive-breaking characters (`;`, whitespace) now raise a build-time `ValueError` at `Site.__init__` instead of silently reaching the emitted CSP. `docs/Foundational/DESIGN-NOTES.md`. Landed in code (`arklight/api.py`) with `tests/test_csp.py` covering it (23 tests, all passing; full suite 2749 passed as of this confirmation). **Maintainer confirmed (2026-09-24): this is a plain bug fix and does not get a dedicated version slot or a `pyproject` bump** -- it stays folded into the existing history the way `0.06505`'s row already described ("skips the unconfirmed `0.06504` draft slot"); this row exists only as the permanent record that the code is real and covered, not as an occupied version number. No code changed by this confirmation pass | DONE |
| v0.0670x **(draft -- version slot unconfirmed)** | Capability fix: unconditional build-time notice for unknown props -- `arklight/backend/html/attrs.py`'s `_attr_string` silently compiled any unrecognized prop key into a generic `data-{name}` attribute (so nothing a user writes disappears from the output), but gave the author nothing to notice a typo by: a correct prop and a typo'd one compile to visually-identical `data-*` markup. Now raises a `[ARKlight ALPHA]`-marked `warnings.warn(...)` at the point of compilation -- same "always prints, never gated behind `--verbose`/`--narrate`" contract as any other alpha-limitation notice (`arklight/cli/main.py`'s `_print_alpha_warnings`) -- naming the prop, the node type, and a structural path (`arklight/ir/validate.py`'s existing `"{path}/{type}[{i}]"` convention, now also threaded through `_render_node`/`_render_children`/`_render_show`/`_render_repeat` in `arklight/backend/html/page_render.py`, which previously carried no path of its own). Non-fatal by design -- same as every other `[ARKlight ALPHA]` notice, the build still succeeds; this flags a plausible mistake for the author to go check, it doesn't second-guess a deliberate custom `data-*` attribute. **Caught a real, previously-undetected bug while landing this**: `usemap` (the `<img>` half of the `<img usemap>`/`<map name>` pairing) was missing from `PASSTHROUGH_ATTRS` entirely, so it silently compiled to `data-usemap` -- inert markup, not a functioning image map -- since `v0.003`'s second addendum; fixed alongside (`tests/test_vocabulary_addendum_2.py::test_map_and_area_render` now asserts on the real attribute). `tests/test_html_attrs.py`/`tests/test_html_backend.py` extended; full suite 2749 passed. Landed in code without a version bump or a confirmed slot -- same treatment as `0.06504`'s CSP fix; a maintainer should confirm the slot (or confirm it stays unslotted, same as `0.06504`) | DONE (code) / DRAFT (record) |
...
| v0.06508 | Message/docs fix: the speaker tag in the experimental-API heavy-reliance nudge (`arklight/experimental.py`), its quoted example in `docs/Foundational/EXPERIMENTAL-APIS.md`, and the root `README.md` status block changes from `[Rae ARK]` to `[Rei]`, matching the Rei voice in `docs/Foundational/DESIGN-NOTES.md`. Wording unchanged; the `Rae-ARK` GitHub org name in URLs/slugs, the `LICENSE` copyright line and the `cctv.py` credit are deliberately left alone. `tests/test_cli.py`/`tests/test_experimental_apis.py` updated; full suite 1644 passed. `0.06507` -> `0.06508`; roadmap `v0.065` untouched | DONE |
...
| v0.06601 | Docs-only: four new design proposals filed against `docs/Proposals/` since `v0.066` closed -- `KAIOS-NATIVE-TARGET-PROPOSAL.md` (KaiOS as a target-specialized backend), `AVM-WASM-SANDBOX-PROPOSAL.md` (running third-party packages in a WebAssembly sandbox), `REISCRIPT1-PROPOSAL.md` (Rei the language, ARKlight-native syntax as a DSL alternative to the Python authoring surface), and `REI-DYNAMIC-CLASS-WASM-PROPOSAL.md`. All four filed as **Proposed**, not accepted; no code changed. `docs/Implementation/REI-LANGUAGE-ADDENDUM.md`, `docs/Implementation/PROVIDER-SDK-ADDENDUM.md` and `docs/Implementation/SEARCH-RETRIEVE-DOC-ADDENDUM.md` (since retired as fully rolled up) also landed/updated in this span. Out-of-band, numbered inside the v0.066 -> v0.067 gap, same slot-sharing precedent as the `0.0650x` series | DONE |
| v0.06602 | Bug fix: preamble parser nesting, part 1 -- `arklight/parser/preamble.py` widened to handle nested/indented `# include`/`# define` directives it previously mis-scoped. `0.06601` -> `0.06602` | DONE |
| v0.06603 | Bug fix: preamble parser nesting, part 2 -- follow-up correction; new `arklight/parser/indentation.py` module (indentation-aware line scanning) wired into `arklight/config.py`/`arklight/parser/loader.py`/`preamble.py`. `tests/test_indentation.py` (new). `0.06602` -> `0.06603` | DONE |
| v0.06604 | Bug fix: `arklight search` extended to cover Platform API entries (`arklight/cli/search.py`); `tests/test_search.py` extended. `0.06603` -> `0.06604` | DONE |
| v0.06605 | Bug fix: `arklight search --retrieve-doc` extended to render Markdown output directly plus a new `--section` flag (`arklight/cli/doc_retrieval.py`, new `arklight/cli/mdrender.py`, `arklight/cli/main.py` wiring); `tests/test_doc_retrieval.py`/`tests/test_mdrender.py` (new) extended. Docs brought back in sync in the next patch below. `0.06604` -> `0.06605` | DONE |
| v0.06606 | Docs-only: `docs/Foundational/AUTHORING-GUIDE.md`/`CLI-REFERENCE.md`/`PLATFORM-APIS.md` and `docs/Implementation/SEARCH-RETRIEVE-DOC-ADDENDUM.md` updated to match `0.06604`/`0.06605`'s search/doc-retrieval changes. No code changed. `0.06605` -> `0.06606` | DONE |
| v0.06607 | Bug fix: user-defined component breakage during dev builds and hydration -- `arklight/backend/js/runtime/repeat.py`, `arklight/ir/components.py`, `arklight/parser/loader.py`; `tests/test_loader.py` extended. `0.06606` -> `0.06607` | DONE |
| v0.06608 | Bug fix: PWA service-worker injection bugs -- `arklight/pwa.py` hardened; `tests/test_pwa.py`/`tests/test_experimental_apis.py` extended. `0.06607` -> `0.06608` | DONE |
| v0.06609 | Bug fix: HTML page-render/search serialization mismatch, plus `arklight search --near` flag fixes -- `arklight/backend/html/page_render.py`, `arklight/cli/search.py`, `arklight/search/engine.py`; `tests/test_html_page_render.py`/`tests/test_search.py`/`tests/test_js_vocabulary_v0061.py`/`tests/test_vdom_4.py` extended. `0.06608` -> `0.06609` | DONE |
| v0.06610 | Bug fix: htmx wiring in the JS backend hardened to play nicely with the rest of the reactive runtime -- `arklight/backend/js/render.py`; `tests/test_htmx_5.py` extended. Full suite 2320 passed. `0.06609` -> `0.06610`; roadmap `v0.067` untouched (JS vocabulary stage 7/10 and `Provider` stage 4/6 still PLANNED) | DONE |
| v0.06611 | Bug fix: bracket-nesting indentation, part 3 -- follow-up correction to `0.06603`'s `arklight/parser/indentation.py`: a closing-only line must now be flush with the line that opened the bracket it closes (part 2 exempted any closing-only line outright, whatever its indentation); new nesting-depth cap (`DEFAULT_MAX_NESTING_DEPTH = 8`) raises `TreeNestingTooDeepError`, a `BracketIndentationError` subclass. `tests/test_indentation.py` extended; `docs/Foundational/AUTHORING-GUIDE.md` updated. `0.06610` -> `0.06611` | DONE |
| v0.06612 | Capability fix: JS vocabulary addendum stage 7/10 (the list-scalar derivations catalog, the `v0.067` slot's second and last piece; `Provider` stage 3 already shipped as `0.06518`) -- 9 new `Derive.*` kinds reducing a list-valued `State(...)`/`Computed(...)` to a scalar: `list_length`, `list_min`/`list_max`/`list_average`, `list_first`/`list_last`, `list_includes`, `list_any`/`list_all` (one of the six `compare` operators plus a literal, never a callback). A non-list reads as an empty list; numeric kinds read each element as `Number(x) || 0` (numbers/strings/booleans only); the three boolean-returning kinds can feed `Show(Predicate.truthy(...))` directly. New `arklight/ir/js_list.py` is the build-time twin, reproducing `Number(string)`'s grammar and `===` equality rather than Python's looser rules; `Bind(...)`'s HTML pre-fill of a missing value now spells `null`, not `None`. `tests/test_js_vocabulary_v0067.py` (265 tests, incl. a ~15,000-case Node parity sweep and a 500,000-item stack-depth check); full suite 2596 passed. `0.06611` -> `0.06612`; closes roadmap `v0.067` | DONE |
| v0.067   | Milestone closed -- both pieces of the `v0.067` slot are DONE, so the package version rolls up from the `0.0651x`/`0.0661x` increments to `0.067`: `Provider` stage 3/6, JS backend emission (`window.ARKLIGHT_PROVIDER`, `0.06518`) and JS vocabulary addendum stage 7/10, the list-scalar derivations catalog (`0.06612`). `docs/version history/v0.067.md`. Full suite 2596 passed | DONE |
| v0.064-v0.070 (remainder) | JS vocabulary addendum, stage 10 of 10 remains (stages 6-9 -- the predicates catalog, the list-scalar derivations catalog, the cross-language numeric batteries, and the formatting/case catalog -- shipped as `0.06517`/`0.06612`/`0.068`/`0.069` and closed out `v0.066`/`v0.067`/`v0.068`/`v0.069`; the rest: capstone `pluralize`/`random_int`, both flagged as needing a design decision first -- `pluralize` needs an irregular-word table, `random_int` breaks the build-time/client agreement) -- `docs/Implementation/JS-VOCABULARY-ADDENDUM-v0.070.md`; per-stage `docs/version history/` previews marked PLANNED until each lands. `v0.070` additionally carried the tail of `Provider`'s six-stage ladder (formerly `docs/Implementation/PROVIDER-SDK-ADDENDUM.md`, since retired -- design record now `docs/Foundational/PROVIDER-SDK.md`), one stage per version -- accepted, independent piece of work sharing this range's milestone slots; stages 1-6 shipped as `0.06514`/`0.06516`/`0.06518`/`0.06613`/`0.06614`/`0.06616` -- `Provider`'s own ladder is now fully closed, only JS vocabulary stage 10/10 keeps this row PLANNED | PLANNED |
| v0.06613 | Capability fix: `Provider`, stage 4 of 6 (the `v0.068` slot's first-shipped piece; JS vocabulary stage 8/10 in the same slot is still PLANNED) -- external script loading. `Page(scripts=[{"src": "https://.../sdk.js", ...}])`, validated in `arklight.ir.validate._validate_page_head_extensions` (non-empty list of non-empty `{attribute: value}` dicts, `src` required, a `"javascript:"` `src` rejected outright), rendered as verbatim `<script ...></script>` tags at the end of `<head>` by `arklight/backend/html/head_meta.py`. Gated as its own `provider-scripts` entry in `arklight/experimental.py` (`upstream_candidate=False`), independent of `provider-integration` -- `arklight/ir/build.py` records the `ExperimentalUsage` off `Page(...)`'s own `scripts` prop, not off a declared `Provider`, so a page can carry either without the other. `arklight/backend/html/csp.py`'s docstring updated: unlike `css-import`, an external script `src` is a real `script-src` concern and needs `Site(trusted_script_origins=[...])` to actually load. `tests/test_provider.py` (53 -> 65 tests)/`tests/test_html_head_meta.py` (13 -> 16 tests) extended; full suite 2815 passed. `0.06612` -> `0.06613`; out-of-band, no roadmap row, roadmap `v0.068` untouched (JS vocabulary stage 8/10 remains) | DONE |
| v0.068   | Milestone closed -- both pieces of the `v0.068` slot are DONE, so the package version rolls up from the `0.06613` increment to `0.068`: JS vocabulary addendum stage 8/10, the cross-language numeric batteries catalog (6 new `Derive.*` kinds: `lerp`, `midpoint`, `saturating_add`/`saturating_subtract`, `value_or`, `first_present`) and `Provider` stage 4/6, external script loading (already shipped as `0.06613`, pulled into this milestone). Every new kind is a pure registry-fragment addition -- no new IR node, no parser -- with a build-time Python mirror in `arklight/ir/build.py` (two new `arklight/ir/js_numeric.py` helpers, `js_lerp`/`js_midpoint`, for the two edge-case-bearing kinds) so server-rendered `Bind(...)` text agrees with the client recompute. `saturating_add`/`saturating_subtract` take literal `min`/`max` bounds (`min` above `max` is a build error); `value_or`'s `fallback` and `first_present`'s empty test both follow "`null`/`undefined`/empty-string counts as absent". `tests/test_js_vocabulary_v0068.py` (new); full suite 2834 passed. `docs/version history/v0.068.md`. `0.067` -> `0.068` | DONE |
| v0.06614 | Capability fix: `Provider`, stage 5 of 6 (the `v0.069` slot's first-shipped piece; JS vocabulary stage 9/10 in the same slot is still PLANNED) -- `arklight search <provider-name>` integration. A declared provider's contract (name, capabilities, and the read-only `window.ARKLIGHT_PROVIDER` runtime hook) is now discoverable through `arklight search`: `arklight/provider.py` gains `PROVIDER_REGISTRY`/`register_provider()`/`resolve_provider()` (exact, case-insensitive lookup); `Site(provider=...)` registers its declaration; `arklight/cli/search.py` checks provider names after component and closed-vocabulary lookups, so a component name always wins a collision. Standalone limitation: `arklight search` doesn't load a project's site file, so a provider (like a user-defined component) is only visible in a process that has already constructed its `Site`. No typo suggestions for provider labels -- they're free strings, so fuzzy matching would be guessing; the roadmap's original wording implied reusing the typo machinery, exact match was chosen instead. `tests/test_provider_search.py` (15 new tests); full suite 2849 passed. `0.06613` -> `0.06614`; out-of-band, no roadmap row, roadmap `v0.069` untouched (JS vocabulary stage 9/10 remains) | DONE |
| v0.06615 | Capability fix: JS vocabulary addendum stage 9/10 (the formatting/case catalog, the `v0.069` slot's second and last piece; `Provider` stage 5 already shipped as `0.06614`) -- 7 new `Derive.*` kinds, each taking one name and no literal arguments: `to_ordinal`, `humanize_bytes`, `humanize_duration`, `to_snake_case`, `to_camel_case`, `to_kebab_case`, `to_title_case`. Same one-fragment-file-plus-registry-line shape as stage 8; case-splitting rules (split on non-letter/digit characters and at case changes, digits treated as word-ending in some positions) are a design choice, not a spec requirement. Bug fixed along the way: `js_numeric.py` had its own `repr()`-based number-to-string helper that kept a trailing `.0` on whole floats; it now reuses the JavaScript-faithful helper already in `js_string.py` (not previously JS-visible, since the affected path only ran for values at or above `1e21`). `humanize_bytes` rounds a value that would show `1024 KB` up to `1 MB`. `tests/test_js_vocabulary_v0069.py` (66 new tests, incl. a ~130-sample Node parity sweep); full suite 2915 passed. `0.06614` -> `0.06615`; closes roadmap `v0.069` | DONE |
| v0.069   | Milestone closed -- both pieces of the `v0.069` slot are DONE, so the package version rolls up from the `0.0661x` increments to `0.069`: JS vocabulary addendum stage 9/10, the formatting/case catalog (7 new `Derive.*` kinds, `0.06615`) and `Provider` stage 5/6, `arklight search` integration (`0.06614`). `docs/version history/v0.069.md`. Full suite 2915 passed | DONE |
...
**User-defined functions** -- per maintainer confirmation, this item is
DONE and no longer belongs in this list. It previously sat here as a
placeholder for reusable *logic* a project can register and have the
compiler treat as a first-class name (parallel to how `Derive.*`/
`Action.*` are each a closed, described vocabulary rather than an
arbitrary expression string), as opposed to reusable *markup*. Not to
be confused with a page function (`@site.page("/")`) or a component's
own `render_fn`, both of which are already "user-defined functions" in
the plain Python sense.
...
## v0.067 -- Milestone rollup: package version `0.06612` -> `0.067` (DONE)

Both pieces of the `v0.067` slot -- `Provider` stage 3/6 (`0.06518`)
and JS vocabulary addendum stage 7/10, the list-scalar derivations
catalog (`0.06612`) -- have been DONE since `0.06612` landed;
`docs/version history/v0.067.md` had already noted the rollup was
pending and pointed here for when it happens. `pyproject.toml`'s
`version` field bumped from `0.06612` to `0.067`; no code changed.
`docs/version history/v0.067.md`'s Status line updated to match. Same
"milestone closed, increments roll up to the plain version" treatment
`v0.066` got.

Also, per maintainer confirmation: **user-defined functions** is DONE.
Removed from the "Planned, not yet scheduled to a version" list in the
Snapshot section above, where it had been sitting as a placeholder note.
No design doc or implementation detail is recorded here for it beyond
that confirmation -- per instruction, not re-derived or investigated
further.

## v0.06612 -- Capability fix: JS vocabulary addendum stage 7/10, list-scalar derivations catalog (DONE)

The second and last piece of the `v0.067` slot (`Provider` stage 3 already
shipped as `0.06518`; see `docs/version history/v0.067.md`) -- 9 new
`Derive.*` kinds that reduce a list-valued `State(...)`/`Computed(...)` to a
scalar: `list_length`, `list_min`/`list_max`/`list_average`,
`list_first`/`list_last`, `list_includes`, `list_any`/`list_all`
(`op`/`value` from the existing six `compare` operators, never a callback).
A value that isn't a list reads as an empty list (`Array.isArray`); numeric
kinds read each element as `Number(x) || 0`, narrowed to numbers/strings/
booleans. `list_includes`/`list_any`/`list_all` return a boolean, so any can
feed `Show(Predicate.truthy(...))`.

New `arklight/ir/js_list.py` is the build-time twin -- same role as
`js_numeric.py`/`js_string.py`/`js_predicate.py` -- reproducing
`Number(string)`'s grammar and `===` equality rather than Python's `float()`
or `1 == True`, so `_evaluate_derivation` (`arklight/ir/build.py`) agrees
with the shipped fragment. New `_validate_list_derivation_args`
(`arklight/ir/validate.py`) checks `list_includes`/`list_any`/`list_all`'s
literal arguments, sharing its JSON-scalar check with `Predicate.one_of
(...)`'s existing one instead of duplicating it. `Bind(...)`'s HTML pre-fill
of a missing value now spells `null`, not Python's `None`
(`arklight/backend/html/page_render.py`) -- needed for `list_first`/
`list_last` of an empty list, and it also corrects an unset `State(...)`'s
own pre-fill to match the client. Every fragment loops/indexes rather than
spreading a list into `Math.min`/`Math.max` (which throws past roughly 100k
items) or using `.at()` (missing pre-2022), checked against a 500,000-item
list in Node.

`tests/test_js_vocabulary_v0067.py` (new, 265 tests): API, registry/fragment
lockstep, validation (bad/boundary literal arguments, every compare operator,
arity/deps, the shared `one_of` check), exact build-time initial values,
`js_list.py`'s number/equality helpers against JavaScript's own rules,
HTML pre-fill (incl. the `null` spelling and a boolean-kind `Show` guard),
JS-backend shipping (only the used kind ships, no `eval`/spread/`.at`), a
500,000-item stack-depth check, and a roughly 15,000-case Node parity sweep --
confirmed to actually catch a mismatch by deliberately breaking the Python
mirror five different ways and rerunning it. Full suite 2596 passed (was
2320 on `0.06610`). `0.06611` -> `0.06612`; out-of-band, no roadmap row, closes
roadmap `v0.067` (`docs/version history/v0.067.md`, `ARCHITECTURE.md`'s
`v0.065`-`v0.070` row).
...
piece, JS vocabulary stage 7/10 (list-scalar derivations), shipped next as
`0.06612` -- see that entry below. `v0.067` itself is closed as of `0.06612`,
still as an out-of-band increment rather than a rolled-up `0.067` package
version.
...
First-shipped piece of the `v0.067` slot; JS vocabulary stage 7/10 (list-scalar
derivations) stays PLANNED in the same slot.

**What went in.** `_provider_config_js(provider)` in
`arklight/backend/js/render.py`, called from `_build_runtime_js` right after the
devtools console reminder, emits
...
when `ir.provider` is set, and nothing otherwise. Values go through
`json.dumps` (ASCII-escaped), never string-built into code.

**Decisions worth a second look.**

- *Where it lives.* `docs/version history/v0.067.md` says "JS backend
  emission" and the proposal says "mirroring `data-ark-state`". A data attribute
  would put Provider data in every page; a `window` global in `arklight.js`
  keeps pages/stylesheet byte-identical (stage 1's invariant, still tested) and
  follows the existing `window.ARKLIGHT_ON_ERROR` convention. It is assigned
  inside the runtime IIFE before anything a `ScriptExtension` appends after it.
- *Site-wide, not per page.* `Site(provider=...)` is one provider per site, so
  "which capabilities a page uses" collapses to "which the site declared".
- *No DOM hooks / state keys.* The proposal's section 3 says the blob describes
  "what DOM hooks/state keys they're wired to". Nothing can author that yet
  (stage 4 is the external `<script src>`; there is no capability-wiring prop),
  so the blob holds only `name` and `capabilities`. Adding fields later is
  backward compatible.
- *Gate is `ir.provider`, not the devtools-reminder flag.* Turning the console
  reminder off must not remove config a site's own code reads.
- `docs/Implementation/PROVIDER-SDK-ADDENDUM.md` was never committed; scope
  followed `v0.067.md` and the proposal, as in stages 1-2. (Both since retired
  now that all six stages have shipped -- design record
  `docs/Foundational/PROVIDER-SDK.md`.)

**Known.** The `.arklight` binary format doesn't round-trip `provider` (stage
2's note), so a build from a decoded `.arklight` file emits no config object.

Full suite 2266 passed (was 2251). `0.06517` -> `0.06518`.
...
Numbered by the `0.0650` + decimals rule like `0.06501`-`0.06514`: an out-of-band
increment tracked here and in `CHANGELOG.md`, not a roadmap row (`ARCHITECTURE.md`
has none for it; `V1-DEFINITION.md` scopes CLI conveniences beyond `build` out of
`v1.0`). Roadmap `v0.065` untouched. It also has its own user-facing note,
`docs/version history/v0.06515.md` (precedent: `v0.0431.md`, `v0.0641.md`), which
`arklight --upgrade-alpha` prints once.
...
**Spec gap, found while doing it (now historical).** The proposal and twelve
other docs pointed at `docs/Implementation/PROVIDER-SDK-ADDENDUM.md` for the
six-stage ladder. That file was never committed to any branch. The stages were
recoverable from the per-version previews (`v0.065.md`-`v0.070.md`), which is
what scope followed; anything the addendum said beyond them is unknown. Both
the proposal and this addendum reference have since been retired now that all
six stages shipped -- see `docs/Foundational/PROVIDER-SDK.md`.
...
The ELIZA file under `docs/reference/eliza/` is an original from-scratch implementation (its header says so), kept as design reference only;
