...
| v0.067   | Milestone closed -- both pieces of the `v0.067` slot are DONE, so the package version rolls up from the `0.0651x`/`0.0661x` increments to `0.067`: `Provider` stage 3/6, JS backend emission (`window.ARKLIGHT_PROVIDER`, `0.06518`) and JS vocabulary addendum stage 7/10, the list-scalar derivations catalog (`0.06612`). `docs/version history/v0.067.md`. Full suite 2596 passed | DONE |\n| v0.064-v0.070 (remainder) | JS vocabulary addendum, stages 9-10 of 10 remain (stages 6-8, the predicates catalog, the list-scalar derivations catalog, and the cross-language numeric batteries, shipped as `0.06517`/`0.06612`/`0.068` and closed out `v0.066`/`v0.067`/`v0.068`; the rest: cross-language formatting/case idioms, capstone `pluralize`/`random_int`) -- `docs/Implementation/JS-VOCABULARY-ADDENDUM-v0.070.md`; per-stage `docs/version history/` previews marked PLANNED until each lands. `v0.069`-`v0.070` additionally carry the tail of `Provider`'s six-stage ladder (`docs/Implementation/PROVIDER-SDK-ADDENDUM.md`), one stage per version -- accepted, independent piece of work sharing this range's milestone slots; stages 1-4 shipped as `0.06514`/`0.06516`/`0.06518`/`0.06613`, stages 5-6 PLANNED | PLANNED |
