...
| v0.064-v0.070 | JS vocabulary addendum, stages 4-10 of 10 (math/string/list-scalar derivation catalogs, predicates catalog, cross-language "batteries included" numeric/formatting idioms, capstone `pluralize`/`random_int`) -- `docs/Implementation/JS-VOCABULARY-ADDENDUM-v0.070.md`; per-stage `docs/version history/` previews marked PLANNED until each lands. `v0.064` additionally carries `arklight search --retrieve-doc` (`docs/Implementation/SEARCH-RETRIEVE-DOC-ADDENDUM.md`); `v0.065`-`v0.070` additionally carry `Provider`'s six-stage ladder (`docs/Implementation/PROVIDER-SDK-ADDENDUM.md`), one stage per version -- both accepted, independent pieces of work sharing this range's milestone slots | PLANNED |
