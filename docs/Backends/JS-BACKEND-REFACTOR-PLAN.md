...
| vdom-7 | Per-item list rendering (`Repeat`) + conditional show/hide (`Show`). The two biggest remaining lifts per `docs/DESIGN-NOTES.md`'s own `v0.044` write-up; needs keyed-children routing through the vendored `patch()` (see `docs/new js backend proposal/ARCHITECTURE-VDOM.md` §6.2-6.3 for the keying/vnode-swap proposal this stage should implement against). **Landed:** `Repeat` implements §6.2's value-keyed `patch()` routing as scoped; `Show` deliberately departs from §6.3's vnode-swap proposal (hydration-safety reasons -- see `docs/Backends/REFACTOR-INDEX.md` row 15 for the full account and `arklight/backend/js/runtime/show.py`'s docstring for the mechanics) in favor of a native `hidden`-attribute toggle. | vdom-4, vdom-6 | **Done** |
