...
| 15 | `vdom-7` | Per-item list rendering (`Repeat`) + conditional show/hide (`Show`); keyed-children routing through the vendored `patch()`. **Landed:** `arklight.api.Repeat(name, template=...)`/`Show(predicate, ...)` (plus `RepeatItem.value()`/`RepeatItem.index()` and `Predicate.truthy(...)`/`Predicate.falsy(...)`) join `arklight/ast/nodes.py` (`ItemIndexRef`, `PredicateRef`) and `arklight/ir/schema.py` (`SCHEMA["Repeat"]`/`SCHEMA["Show"]`, a new `PREDICATE_REGISTRY`) as real, renderable content -- unlike `Computed`/`Watch`, they stay in the tree and render wherever they're placed, so Validation (`arklight/ir/validate.py`) gained dedicated recursive checks (`_validate_repeat_template`, `_validate_show_declaration`/`_validate_predicate_ref`) rather than the page-scoped-declaration path `Watch` uses. The HTML backend (`arklight/backend/html/page_render.py`) renders a `Repeat`'s current items as real, unrestricted markup (`_render_repeat`/`_substitute_item_refs`, so a JS-disabled visitor sees the genuine list) plus a JSON `data-ark-repeat-template` spec (`_repeat_template_spec`, narrower: `class`/`id`/one `on_click=` per node) the client uses to build *new* items after `Action.append(...)`; a `Show`'s children always render (`_render_show`), toggled via the native `hidden` attribute rather than omitted markup, so there's real content to reveal with no JS. On the JS side (`arklight/backend/js/runtime/repeat.py`, `show.py`), `Repeat` does route through the vendored snabbdom `patch()` as this row originally scoped -- keyed by each item's own `JSON.stringify(item)` value, not index, so an `Action.remove(...)` doesn't cause every later item to be misdiagnosed as changed -- but `Show` deliberately does **not**: the vendored core has no dedicated hydration pass, so swapping `Show`'s own vnode `sel` (the SS6.3 proposal) would duplicate or destroy server-rendered content on the very first toggle; `renderRepeat` instead solves the equivalent problem for lists by "adopting" the server-rendered DOM into a matching vnode tree (`arkAdoptVnode`) on its first call, before ever calling `patch()` for real. `render.py`'s `_collect_usage`/`_build_runtime_js` gained `has_repeat`/`has_show` flags so `RENDER_REPEAT_JS`/`RENDER_SHOW_JS` ship only on a page that uses one or the other, same "only ship what's used" discipline as `vdom-6`. See `tests/test_vdom_7.py` (21 tests) for dedicated coverage; full suite 970 passed, no regressions. | `JS-BACKEND-REFACTOR-PLAN.md` -> `vdom-7`; `docs/new js backend proposal/ARCHITECTURE-VDOM.md` SS6.2-6.3 | `vdom-4`, `vdom-6` | **Done** |
