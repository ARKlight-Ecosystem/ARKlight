...
| [`USER-DEFINED-ERROR-HANDLING-PROPOSAL.md`](USER-DEFINED-ERROR-HANDLING-PROPOSAL.md) | Proposal for opt-in, user-defined error handling: `# include <errhanlib.ARKlight>` activates a three-bracket comment directive, `# [catch what] [sink] [message]`, placed on the line directly above any ARKlight-recognised function it wraps (the target is implicit). Sinks are the compiler's own log (`ARKlight Log`) and the generated JS (`JS Log`, routed through the `0.06505` `arkReportError` funnel). Log-only: it never swallows an error and does not touch Python's `try`/`except`. Also borrows the `<x>lib.ARKlight` naming for future platform libs. **Not accepted; requested slot `v0.079` (already held by Miko Stage A); the meaning of the first bracket is an open question.** |
