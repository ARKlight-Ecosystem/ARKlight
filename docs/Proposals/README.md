...
| [`CSP-TRUSTED-ORIGIN-INJECTION-BUGFIX.md`](CSP-TRUSTED-ORIGIN-INJECTION-BUGFIX.md) | Bug fix for `trusted_script_origins` CSP directive injection: `_render_csp_meta_tag` spliced entries verbatim into `script-src`, letting `'unsafe-inline'`/directive-breaking strings silently undo the strict-CSP guarantee `Site.__init__`'s own docstring promised. Out-of-band issue-triage fix per `V1-DEFINITION.md`, not a milestone feature. **Implemented, alpha.** |
...
| [`COMPONENT-CALL-DIAGNOSTICS-PROPOSAL.md`](COMPONENT-CALL-DIAGNOSTICS-PROPOSAL.md) | Capability fix for issue-register #5/#32: a positional call to a user-defined component (`Stat("a", "b")`) leaked Python's own `takes 0 positional arguments` `TypeError`, and a `props=` contract that disagreed with its render function's signature leaked a raw `TypeError` from inside expansion. Both now raise `ComponentError` with the component, the rule, the declared props, a by-name example and the call's `file:line`; the signature check is argument *binding* only, so it can never reject a call that would have worked. **Implemented as of `0.06506`.** |
...
| [`ACTION-VALUE-FROM-STATE-PROPOSAL.md`](ACTION-VALUE-FROM-STATE-PROPOSAL.md) | Capability fix for issue-register #7: `Bind("name")` as an `Action.set`/`Action.append` argument, resolved from live state when the action runs, so `[type a task] [Add]` is expressible. Closed-vocabulary throughout: opt-in per argument (`ActionSpec.state_args`), build-time validated, no eval. **Implemented as of `0.06503`.** |
...
| [`REI-LANGUAGE-PROPOSAL.md`](REI-LANGUAGE-PROPOSAL.md) | Proposal for the Rei *language* (`.rei`), a native source frontend for ARKlight -- distinct from Rei the compiler narrator (`REI-COMPILER-NARRATOR-PROPOSAL.md`, shipped `0.06510`). `.rei` -> Rei AST -> ARK AST, changing nothing below the ARK AST: `.rei` site files, a data-only `arklight.config.rei`, a spec that tracks ISO C99's shape and philosophy (not the C language), a Dart-like tree surface, and a four-stage ladder. As filed it is a playground behind an alpha-only "Fun" API tier with a `fun.py` registry. **Not accepted as written -- but the maintainer has since decided Rei is ARKlight's official native source language, added beside Python (which stays), so the playground/Fun-tier framing is superseded and its gating is an open question (Q11).** Re-verified against `alpha` @ `5a15ebb`. |
