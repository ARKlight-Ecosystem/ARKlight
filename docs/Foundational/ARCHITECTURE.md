...
| v0.064 | `arklight search --retrieve-doc`, a doc-tree retrieval mode added to the existing `search` subcommand (root/folder index printing, `--file NAME` full-file retrieval) -- accepted, a single, self-contained CLI addition with no separate implementation addendum (a retrospective one was later filed, then retired as fully rolled up); see `docs/version history/v0.064.md`. **Plus** JS vocabulary addendum, stage 4 of 10 (math derivations catalog, 21 `Derive.*` kinds) -- staged in `docs/Implementation/JS-VOCABULARY-ADDENDUM-v0.070.md`, **shipped** out-of-band as `0.06509` after the capability fixes that paused it; see `docs/version history/v0.064.md`. Two independent pieces of work sharing one milestone slot, same precedent as `v0.041` (CLI/pipeline hardening + JS vocabulary addenda I & II) -- unlike `v0.041`, this slot's two pieces landed at different times, retrieve-doc at `v0.064` and JS vocab stage 4 later, as `0.06509` | DONE |
| v0.065-v0.070 | JS vocabulary addendum, stages 5-10 of 10 (string/list-scalar derivation catalogs, predicates catalog, cross-language "batteries included" numeric/formatting idioms, capstone `pluralize`/`random_int`) -- staged in `docs/Implementation/JS-VOCABULARY-ADDENDUM-v0.070.md`; **stage 5 (string derivations catalog, 18 `Derive.*` kinds), stage 6 (predicates catalog, 8 new `Predicate.*` kinds) and stage 7 (list-scalar derivations catalog, 9 new `Derive.*` kinds) have shipped** out-of-band as `0.06513`, `0.06517` and `0.06612`, stages 8-10 remain PLANNED; **the `v0.066` slot is closed** (predicates catalog + `Provider` stage 2, package version `0.066`); **the `v0.067` slot is also closed** (`Provider` stage 3 as `0.06518` and JS vocab stage 7 as `0.06612`, since rolled up to a plain `0.067` package version, same as `v0.066`) -- **interleaved with** `Provider`, an experimental, barebones interface for a site to declare it talks to an external service at runtime (contract + experimental gating, IR/validation, JS config emission, external-script loading, capability-enum finalization); its six-rung ladder (stages 1-6, one per version in this range) is now **fully SHIPPED** (stage 6 landed as the out-of-band increment `0.06616`) -- see the settled design record `docs/Foundational/PROVIDER-SDK.md`, graduated from the now-retired proposal and addendum -- **and, at `v0.065` specifically, a third interleaved piece:** Rei, the compiler narrator (`--narrate` flag on `arklight build` + a `rei` config section), accepted from `docs/Foundational/DESIGN-NOTES.md` and shipped as `0.06510`, one version, no ladder; each stage's `docs/version history/vX.md` is a marked **PLANNED** preview until its stage actually lands, see that directory's own README for the convention | PLANNED |
...
| v0.081-v0.084 | Rei language -- a native, compiler-owned `.rei` source frontend lowering to the same ARK AST Python authoring produces, added *beside* Python (which stays the default), plus a data-only `arklight.config.rei`; accepted, staged as a four-rung ladder (maturity gate/package skeleton/RNI -> lexer/parser/tree subset/entry-point -> config -> exceptions/Platform-APIs-as-interface) in `docs/Implementation/REI-LANGUAGE-ADDENDUM.md`; the Compute stage (`const`/functions/a minimal evaluator) is unscheduled, blocked on an open build-time-evaluator design question | PLANNED |
