...
| [`PROVIDER-SDK.md`](PROVIDER-SDK.md) | Settled design record for `Provider` (`v0.065`-`v0.070`, six-rung ladder now fully SHIPPED) -- the barebones external-service interface, the finalized capability vocabulary plus its `custom:`-prefixed escape hatch, external script loading, and scope boundaries. Graduated from the now-retired `docs/Proposals/PROVIDER-SDK-PROPOSAL.md`/`docs/Implementation/PROVIDER-SDK-ADDENDUM.md`. |
