...
| [`USER-DEFINED-ERROR-HANDLING-PROPOSAL.md`](<Proposals/USER-DEFINED-ERROR-HANDLING-PROPOSAL.md>) | Proposal for opt-in user-defined error handling: `# include <errhanlib.ARKlight>` enables a three-bracket comment directive placed directly above any ARKlight-recognised function, logging a custom message to the compiler or the generated JS without swallowing the error or touching Python's `try`/`except`. **Not accepted**; requested slot `v0.079` is held by Miko Stage A, and the first bracket's meaning is an open question. |
